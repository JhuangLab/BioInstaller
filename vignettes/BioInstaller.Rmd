...
What's more, BioInstaller provides a different way to download and install your files, softwares, and databases. More detail can be found in another vignette [Write Your Own Configuration File](https://life2cloud.com/tools/bioinstaller/articles/write_configuration_file.html).
...
```{r echo = FALSE}
suppressWarnings(
  DT::datatable(matrix(x, ncol=3), caption = sprintf("Sotware and database supported by BioInstaller (n=%s)", length(x)))
)
```
