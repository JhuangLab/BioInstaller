...
Configuration files in BioInstaller are important. We used these configuration files to stored the software and databases URL, script of installation, and other userful informations. 

Most of configuration files be parsed by [configr](https://github.com/Miachol/configr). Compared with original configr package syntex `#R# R CMD #R#` is a different point. It can be used to mark those R format command. 
...
Variables to control the download and installation steps of softwares and databases deposited on github:
...
- The `make_dir` is the compile directory of software and database. Because the workdir of R default will be setted to `download.dir`, and need be changed to `make_dir` finish `install` steps.
...
Variables to control the download and installation steps of softwares and databases not be deposited on github:

- `github_url` be replaced by `source_url` 
- If you want to download mulitple files in the source_url, you need to set `url_all_download` to `true`.
- [rvest](https://cran.r-project.org/package=rvest) and [RCurl](https://cran.r-project.org/package=RCurl) packages can be used to parse the version infomation of softwares or databases of non-github.
...
Besides, BioInstaller use [configr](https://github.com/Miachol/configr) `glue` to reduce the length of files name. It can help you to use less word to storage more files name.
...
To resolve the some softwares dependence, BioInstall using the `{{key:value}}` format expression, and get its value from BioInstall `BIO_SOFWARES_DB_ACTIVE` database.

For example, `htslib` is the dependce of Pindel, and we use `./INSTALL {{htslib:source.dir}}` as the install step of Pindel. In the session of R, the value of `{{htslib:source.dir}}` will be replaced by the real value stored in `BIO_SOFTWARES_DB_ACTIVE` or `db` in `install.bioinfo` function.
