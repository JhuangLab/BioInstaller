---
title: "Plugins of bioshiny Application"
author: "Jianfeng Li"
date: "`r Sys.Date()`"
output:
  ...
  pdf_document:
    toc: true
vignette: >
  %\VignetteIndexEntry{Plugins of BioInstaller Shiny Application}
...
Both the R files and plugin files (TOML format) are supported to extend and add new functions of bioshiny application. Users can modifed the value of shiny_plugin_dir to use a custom plugins library. The plugins filename: shiny.easy_project.parameters.toml, shiny.maftools.parameters, etc. Now we use the "shiny.{toolname}.parameters.toml" rule to search the plugin file, or you can modifed the line 84 of `global_bar.R` to change the rule.
